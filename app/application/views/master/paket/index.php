        ...
        <?php if ($form["type"] != "hidden" && !$isHideFromTable) : ?>
            <?php if ($index == 0) : ?>
                RFL_COLUMNS.push({
                    ...
                    render: function(data, type, row, meta) {
                        return /* html */ `
                            <a class="btn default btn-outline image-popup-vertical-fit el-link" href="${data ? " <?= base_url(LOKASI_PAKET_GAMBAR) ?>" + data : "<?= base_url(LOKASI_LOGO) ?>" }">
                                <img src="${data ? " <?= base_url(LOKASI_PAKET_GAMBAR) ?>" + data : "<?= base_url(LOKASI_LOGO) ?>" }" alt="${row.nama}" class="" width="50" height="50" />
                            </a>
                            `
                    }
                })
