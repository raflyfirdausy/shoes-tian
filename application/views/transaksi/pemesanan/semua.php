<div class="container-fluid">
    <?php $this->load->view($RFL_TABLE) ?>
</div>

<?php $this->load->view($RFL_MODAL, [
    "modal_id"      => "modal_tambah",
    "modal_title"   => "Form Tambah",
    "modal_form_id" => "form_add",
    "modal_edit"    => false
]) ?>

<?php $this->load->view($RFL_MODAL, [
    "modal_id"      => "modal_edit",
    "modal_title"   => "Form Edit",
    "modal_form_id" => "form_edit",
    "modal_edit"    => true
]) ?>

<script>
    let RFL_COLUMNS = [{
            "data": null,
            "sortable": false,
            className: "text-center align-middle",
            render: function(data, type, row, meta) {
                return meta.row + meta.settings._iDisplayStart + 1;
            }
        },
        {
            "data": "id",
            className: "text-center align-middle",
            render: function(data, type, row, meta) {
                let btnProses = /*html*/ `<button onclick="proses('${row.id}')" class="dropdown-item"><i class="fas fa-spinner"></i> Proses</button> `
                let btnSelesai = /*html*/ `<button onclick="selesai('${row.id}')" class="dropdown-item"><i class="fas fa-check"></i> Selesai</button> `
                let btnTolak = /*html*/ `<button onclick="tolak('${row.id}')" class="dropdown-item"><i class="fas fa-close"></i> Tolak</button> `

                if (row.status_transaksi == "BOOKING") {
                    btnSelesai = ""
                } else if (row.status_transaksi == "DIPROSES") {
                    btnProses = ""
                    btnTolak = ""
                } else if (row.status_transaksi == "SELESAI") {
                    btnProses = ""
                    btnSelesai = ""
                    btnTolak = ""
                } else if (row.status_transaksi == "DITOLAK") {
                    btnSelesai = ""
                    btnTolak = ""
                } else { //? DIBATALKAN
                    btnProses = ""
                    btnSelesai = ""
                    btnTolak = ""
                }


                let result = /* html */ `              
                    <div class="dropdown">
                        <button style="width:100%" class="btn btn-sm btn-primary dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            Pilih Aksi
                        </button>
                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">                                                                                     
                            <button onclick="lihatDetail('${row.kode_transaksi}')" class="dropdown-item"><i class="fas fa-eye"></i> Lihat Detail</button>                                                                   
                            ${btnProses}
                            ${btnSelesai}
                            ${btnTolak}
                        </div>
                    </div>
                    `
                return result;
            }
        },
    ]

    <?php $index = 0;
    foreach ($FIELD_FORM as $form) : $isHideFromTable = isset($form["hideFromTable"]) ? $form["hideFromTable"] : FALSE; ?>
        <?php if ($form["type"] != "hidden" && !$isHideFromTable) : ?>
            <?php if ($index == 4) : ?>
                RFL_COLUMNS.push({
                    data: "bukti_bayar",
                    className: "text-center align-middle",
                    render: function(data, type, row, meta) {
                        if (data) {
                            return /* html */ `
                            <a class="btn default btn-outline image-popup-vertical-fit el-link" href="${data ? " <?= base_url(LOKASI_BUKTI) ?>" + data : "<?= base_url(LOKASI_LOGO) ?>" }">
                                <img src="${data ? " <?= base_url(LOKASI_BUKTI) ?>" + data : "<?= base_url(LOKASI_LOGO) ?>" }" alt="${row.nama}" class="" width="50" height="50" />
                            </a>
                            `
                        } else {
                            return ""
                        }

                    }
                })
            <?php else : ?>
                RFL_COLUMNS.push({
                    data: "<?= isset($form["name_alias"]) ? $form["name_alias"] : $form["name"] ?>"
                })
            <?php endif ?>
        <?php endif ?>

    <?php $index++;
    endforeach ?>
    RFL_COLUMNS.push({
        "data": "created_at"
    })

    var RFL_TABLE = generateDatatable("table_data", "<?= $URL_GET_DATA ?>", RFL_COLUMNS)
    generateAjaxProses("form_add", "<?= $URL_CREATE_DATA ?>", RFL_TABLE)
    generateAjaxProses("form_edit", "<?= $URL_UPDATE_DATA ?>", RFL_TABLE)
    const hapus = id => hapusData(id, "<?= $URL_DELETE_DATA ?>", RFL_TABLE)
    const modalEdit = id => modalEditAction(id, "<?= $URL_DETAIL_DATA ?>", "modal_edit", <?= json_encode($FIELD_FORM) ?>)

    $(document).ready(() => {

        generateSearchTable("table_data", RFL_TABLE)
        $(".select2").select2()

        <?php foreach ($FIELD_FORM as $form) :
            $idForm             = $form["name"];
            $idFormEdit         = $form["name"] . "_edit";
            $isRequired         = $form["required"] ? "required" : "";
            $isHideFromTable    = isset($form["hideFromTable"]) ? $form["hideFromTable"] : FALSE;
            $ishideFromCreate   = isset($form["hideFromCreate"])    ? $form["hideFromCreate"]   : FALSE;
        ?>
            <?php if ($form["type"] == "select" && $form["options"]["chain"]) :
                $idFormTo       = $form["options"]["to"];
                $idFormToEdit   = $form["options"]["to"] . "_edit";
                $indexChain     = array_search($form["options"]["to"], array_column($FIELD_FORM, "name"));
            ?>
                $("#<?= $idFormTo ?>").change(() => {
                    let id = $("#<?= $idFormTo ?>").val()
                    $("#<?= $idForm ?>").html(`<option value="" selected disabled>Sedang mencari data..</option>`)
                    $.ajax({
                        url: "<?= $form["options"]["data"] ?>" + id,
                        type: "GET",
                        dataType: "JSON",
                        contentType: "application/json; charset=utf-8",
                        success: function(result) {
                            let _dataSelect2 = `<option value="" selected>Pilih <?= strtolower($FIELD_FORM[$indexChain]["label"]) ?> terlebih dahulu</option>`
                            if (result.code == 200) {
                                _dataSelect2 = `<option value="" selected>Pilih <?= strtolower($form["label"]) ?></option>`
                                result.data.forEach((currentValue, index, arr) => {
                                    _dataSelect2 += `<option value="${currentValue.value}">${currentValue.label}</option>`
                                })
                            } else {
                                _dataSelect2 = `<option value="" selected>${result.message}</option>`
                            }
                            $("#<?= $idForm ?>").html(_dataSelect2)
                        },
                        error: function(xhr, ajaxOptions, thrownError) {
                            Swal.fire("Oops", xhr.responseText, "error")
                        }
                    })
                })

                $("#<?= $idFormToEdit ?>").change(() => {
                    let id = $("#<?= $idFormToEdit ?>").val()
                    $("#<?= $idFormEdit ?>").html(`<option value="" selected disabled>Sedang mencari data..</option>`)
                    $.ajax({
                        url: "<?= $form["options"]["data"] ?>" + id,
                        type: "GET",
                        dataType: "JSON",
                        contentType: "application/json; charset=utf-8",
                        success: function(result) {
                            let _dataSelect2 = `<option value="" selected>Pilih <?= strtolower($FIELD_FORM[$indexChain]["label"]) ?> terlebih dahulu</option>`
                            if (result.code == 200) {
                                _dataSelect2 = `<option value="" selected>Pilih <?= strtolower($form["label"]) ?></option>`
                                result.data.forEach((currentValue, index, arr) => {
                                    _dataSelect2 += `<option value="${currentValue.value}">${currentValue.label}</option>`
                                })
                            } else {
                                _dataSelect2 = `<option value="" selected>${result.message}</option>`
                            }
                            $("#<?= $idFormEdit ?>").html(_dataSelect2)
                        },
                        error: function(xhr, ajaxOptions, thrownError) {
                            Swal.fire("Oops", xhr.responseText, "error")
                        }
                    })
                })

            <?php endif ?>
        <?php endforeach ?>
    })

    const gambar = id => {
        location.href = `<?= base_url("master/wisata/gambar/") ?>${id}`
    }
</script>

<script>
    const lihatDetail = (kode) => {
        window.location = "<?= base_url("transaksi/order/detail/kode/") ?>" + kode
    }

    const proses = id => {
        Swal.fire({
                type: 'question',
                title: 'Peringatan',
                html: 'Apakah kamu akan memproses data pemesanan ini ?',
                showCancelButton: true,
                confirmButtonText: 'Ya, Proses',
                cancelButtonText: "Tidak"
            })
            .then((response) => {
                if (response.value) {
                    Swal.fire({
                        title: 'Mohon Tunggu Beberapa Saat',
                        html: 'Sedang memproses orderan',
                        onBeforeOpen: () => {
                            Swal.showLoading()
                            $.ajax({
                                type: "GET",
                                url: "<?= base_url("transaksi/pemesanan/semua/proses/") ?>" + id,
                                contentType: false, //this is requireded please see answers above
                                processData: false, //this is requireded please see answers above
                                dataType: "JSON",
                                success: function(result) {
                                    if (result["code"] == 200) {
                                        Swal.close();
                                        Swal.fire({
                                                type: 'success',
                                                customClass: 'swal-custom',
                                                title: 'Berhasil!',
                                                html: '<pre>' + JSON.stringify(result.message, null, " ").replace(/{|}|,|"/g, "") + '</pre>',
                                                confirmButtonText: 'Tutup',
                                            })
                                            .then((response) => {
                                                RFL_TABLE.ajax.reload(null, false)
                                            })
                                    } else {
                                        Swal.fire({
                                            type: 'info',
                                            customClass: 'swal-custom',
                                            title: 'Info',
                                            html: '<pre>' + JSON.stringify(result.message, null, " ").replace(/{|}|,|"/g, "") + '</pre>',
                                            confirmButtonText: 'Tutup',
                                        })
                                    }
                                },
                                error: function(xhr, ajaxOptions, thrownError) {
                                    Swal.fire("Gagal", xhr.responseText, "error")
                                }
                            });
                        }
                    })
                }
            })
    }

    const selesai = id => {
        Swal.fire({
                type: 'question',
                title: 'Peringatan',
                html: 'Apakah kamu akan menyelesaikan data pemesanan ini ?',
                showCancelButton: true,
                confirmButtonText: 'Ya, Selesaikan',
                cancelButtonText: "Batal"
            })
            .then((response) => {
                if (response.value) {
                    Swal.fire({
                        title: 'Mohon Tunggu Beberapa Saat',
                        html: 'Sedang menyelesaikan orderan',
                        onBeforeOpen: () => {
                            Swal.showLoading()
                            $.ajax({
                                type: "GET",
                                url: "<?= base_url("transaksi/pemesanan/semua/selesai/") ?>" + id,
                                contentType: false, //this is requireded please see answers above
                                processData: false, //this is requireded please see answers above
                                dataType: "JSON",
                                success: function(result) {
                                    if (result["code"] == 200) {
                                        Swal.close();
                                        Swal.fire({
                                                type: 'success',
                                                customClass: 'swal-custom',
                                                title: 'Berhasil!',
                                                html: '<pre>' + JSON.stringify(result.message, null, " ").replace(/{|}|,|"/g, "") + '</pre>',
                                                confirmButtonText: 'Tutup',
                                            })
                                            .then((response) => {
                                                RFL_TABLE.ajax.reload(null, false)
                                            })
                                    } else {
                                        Swal.fire({
                                            type: 'info',
                                            customClass: 'swal-custom',
                                            title: 'Info',
                                            html: '<pre>' + JSON.stringify(result.message, null, " ").replace(/{|}|,|"/g, "") + '</pre>',
                                            confirmButtonText: 'Tutup',
                                        })
                                    }
                                },
                                error: function(xhr, ajaxOptions, thrownError) {
                                    Swal.fire("Gagal", xhr.responseText, "error")
                                }
                            });
                        }
                    })
                }
            })
    }

    const tolak = id => {
        Swal.fire({
                type: 'question',
                title: 'Peringatan',
                html: 'Apakah kamu akan menolak data pemesanan ini ?',
                showCancelButton: true,
                confirmButtonText: 'Ya, Selesaikan',
                cancelButtonText: "Batal"
            })
            .then((response) => {
                if (response.value) {
                    Swal.fire({
                        title: 'Mohon Tunggu Beberapa Saat',
                        html: 'Sedang menyelesaikan orderan',
                        onBeforeOpen: () => {
                            Swal.showLoading()
                            $.ajax({
                                type: "GET",
                                url: "<?= base_url("transaksi/pemesanan/semua/tolak/") ?>" + id,
                                contentType: false, //this is requireded please see answers above
                                processData: false, //this is requireded please see answers above
                                dataType: "JSON",
                                success: function(result) {
                                    if (result["code"] == 200) {
                                        Swal.close();
                                        Swal.fire({
                                                type: 'success',
                                                customClass: 'swal-custom',
                                                title: 'Berhasil!',
                                                html: '<pre>' + JSON.stringify(result.message, null, " ").replace(/{|}|,|"/g, "") + '</pre>',
                                                confirmButtonText: 'Tutup',
                                            })
                                            .then((response) => {
                                                RFL_TABLE.ajax.reload(null, false)
                                            })
                                    } else {
                                        Swal.fire({
                                            type: 'info',
                                            customClass: 'swal-custom',
                                            title: 'Info',
                                            html: '<pre>' + JSON.stringify(result.message, null, " ").replace(/{|}|,|"/g, "") + '</pre>',
                                            confirmButtonText: 'Tutup',
                                        })
                                    }
                                },
                                error: function(xhr, ajaxOptions, thrownError) {
                                    Swal.fire("Gagal", xhr.responseText, "error")
                                }
                            });
                        }
                    })
                }
            })
    }
</script>